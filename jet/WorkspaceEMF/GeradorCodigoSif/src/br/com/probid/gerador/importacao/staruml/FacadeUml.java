...
public class FacadeUml {
	...
	*/
	...
	private static void executaParsing() {
		try {
			HandlerXmiGenericoOld contentHandler = new HandlerXmiGenericoOld();
	...
	*/
	...
	private static void preencheMap(List listaObjetos) {
		...
		while (iterador.hasNext()) {
			...
			if (objeto instanceof UMLAssociationView) {
				alocaRelacionamento((RelacionamentoI)objeto);
			} 
		}
	}
	*/
	...
	/*
	private static void alocaRelacionamento(RelacionamentoI relacionamento) {
		Set listaNome = mapaTabelas.keySet();
		Iterator it = listaNome.iterator();
		while (it.hasNext()) {
			String nomeCorrente = it.next().toString();
			ClasseI classeCorrente = (ClasseI) mapaTabelas.get(nomeCorrente);
			if (classeCorrente.getGuid().compareTo(relacionamento.getGuidHead())==0) {
				//relacionamento.setClasseHead(classeCorrente);
			}
			if (classeCorrente.getGuid().compareTo(relacionamento.getGuidTail())==0) {
				//relacionamento.setClasseTail(classeCorrente);
			}
		}
		...
		while (it.hasNext()) {
			...
			ClasseI classeCorrente = (ClasseI) mapaTabelas.get(nomeCorrente);
			if (classeCorrente.getGuid().compareTo(relacionamento.getGuidHead())==0) {
				classeCorrente.adicionaRelacionamentoFonte(relacionamento);
				break;
			}
