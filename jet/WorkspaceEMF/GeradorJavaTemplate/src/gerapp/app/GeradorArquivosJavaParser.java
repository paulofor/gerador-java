...
import jet.java.dao.DBB;
import jet.java.dao.DaoBase;
import jet.java.dao.DaoBaseI;
...
public class GeradorArquivosJavaParser extends GeradorArquivosBase {
	...
	private void criaArquivosDao(Recursos recursos) throws IOException {
		...
		nomeArquivo = recursos.getConfiguracao().getPathJava() + "//dao//basica//" + recursos.getClasse().getNomeParaClasse() + "DaoBaseI.java";
		if (!this.existe(nomeArquivo)) {
			conteudo = DaoBaseI.create("\n").generate(recursos);
			geraArquivoFonte(conteudo,nomeArquivo);
		}
	}
	...
	public void criaArquivoUnico(Recursos recurso) throws IOException {
		// TODO Auto-generated method stub
		System.out.println("Passou unico");
		String nomeArquivo = recurso.getConfiguracao().getPathJava() + "//modelo//FabricaVo.java";
		String conteudo = FabricaVo.create("\n").generate(recurso);
		geraArquivoFonte(conteudo,nomeArquivo);
		nomeArquivo = recurso.getConfiguracao().getPathJava() + "//dao//DBB.java";
		conteudo = DBB.create("\n").generate(recurso);
		geraArquivoFonte(conteudo,nomeArquivo);
		nomeArquivo = recurso.getConfiguracao().getPathJava() + "//regracolecao//FabricaRegra.java";
		conteudo = FabricaRegra.create("\n").generate(recurso);
		geraArquivoFonte(conteudo,nomeArquivo);
	}
